...
export default class AzureLoginView extends React.Component {
	...
	_handleTokenRequest(e:{ url:string }):any{
		...
		if( code !== null ){
			...
			// The URL changed, but there's not a code in it. If there was an error
			// (for example, that you haven't registered the hardcoded redirect_uri of
			// https://localhost:3000/login from AzureInstance with your app), then
			// that error will be in an error_description parameter in the URL.
			// Retrieve it, and console.warn it so people can see what's going on.
			if (e.url.indexOf("error_description") !== -1) {
				let error_description = e.url.match(/error_description=(.+)&/);
				if (error_description) {
					let err_desc_text = decodeURIComponent(error_description[1].replace(/\+/g, "%20"));
					console.warn(err_desc_text);
				}
			}
		}
  	}
